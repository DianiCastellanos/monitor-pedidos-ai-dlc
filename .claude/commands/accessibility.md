...
When an element receives keyboard focus, it must not be entirely hidden by sticky headers or overlapping panels.
...
Login flows must not rely on cognitive function tests unless copy-paste/autofill is available.
...
Announce errors to screen readers with `role="alert"`, set `aria-invalid="true"` on invalid fields.
...
- [ ] **Target size:** Interactive elements ≥ 24×24px
