...
if [ "$(echo "$DB_PROVIDER" | tr '[:upper:]' '[:lower:]')" = "supabase" ]; then
    echo "[Monitor Pedidos eCommerce] DB_PROVIDER=supabase — omitiendo SQL Server, iniciando app directamente..."
...
echo "[Monitor Pedidos eCommerce] DB_PROVIDER=sqlserver — esperando SQL Server (máx. 10 intentos)..."
ATTEMPTS=0
MAX_ATTEMPTS=10
until sqlcmd -S "$DB_HOST,1433" -U "$DB_USER" -P "$DB_PASSWORD" -C -Q "SELECT 1" > /dev/null 2>&1; do
    ATTEMPTS=$((ATTEMPTS + 1))
    if [ "$ATTEMPTS" -ge "$MAX_ATTEMPTS" ]; then
        echo "[Monitor Pedidos eCommerce] SQL Server no respondió tras $MAX_ATTEMPTS intentos — iniciando en modo degradado."
        break
    fi
    echo "[Monitor Pedidos eCommerce] SQL Server no disponible aún (intento $ATTEMPTS/$MAX_ATTEMPTS), reintentando en 3s..."
    sleep 3
done

echo "[Monitor Pedidos eCommerce] Iniciando app..."
exec dotnet MonitorPedidos.Web.dll
